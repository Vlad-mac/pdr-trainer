let allQuestions = []; let currentQuestions = []; let currentQuestionIndex = 0; let score = 0; let answered = false; let currentTopic = ""; let currentMode = "topic"; // "topic" або "exam" let questionStates = {}; let currentTopicStarted = false; let topicStartTime = null; let topicTimerInterval = null; let elapsedSeconds = 0; let teacherRefCode = null;

const SUPABASE_URL = "https://tsqjfphauhphdksstbob.supabase.co"; const SUPABASE_ANON_KEY = "sb_publishable_hLnSso-oks7c2BNJyneiCA_oNIaGDLU"; const supabaseClient = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

let currentUser = null; let currentProfile = null;

function getReferralCodeFromUrl() { try { const params = new URLSearchParams(window.location.search); const ref = params.get("ref"); return ref ? ref.trim() : null; } catch (error) { console.error("Не вдалося зчитати ref з URL:", error); return null; } }

function saveReferralCode(refCode) { if (!refCode) return; localStorage.setItem("teacher_ref_code", refCode); }

function loadSavedReferralCode() { return localStorage.getItem("teacher_ref_code"); }

function initReferralCode() { const refFromUrl = getReferralCodeFromUrl();

if (refFromUrl) { teacherRefCode = refFromUrl; saveReferralCode(refFromUrl); return; }
...
async function loadQuestions() { try { const response = await fetch("questions.json"); if (!response.ok) { throw new Error("Не вдалося завантажити questions.json"); }

allQuestions = await response.json(); } catch (error) { console.error("Помилка завантаження питань:", error);
...
async function loadUserStatsFromSupabase() { if (!currentUser) return [];

const { data, error } = await supabaseClient .from("user_stats") .select("topic, question_id, correct, created_at, time_spent_seconds") .eq("user_id", currentUser.id) .order("created_at", { ascending: true });
...
async function addStat(topic, questionId, isCorrect) { if (!currentUser) return;
...
function startTopicTimer() { stopTopicTimer(); topicStartTime = new Date(); elapsedSeconds = 0;

topicTimerInterval = setInterval(() => { elapsedSeconds++; const timerEl = document.getElementById("topic-timer"); if (timerEl) { timerEl.textContent = formatTime(elapsedSeconds); } }, 1000);

}

function stopTopicTimer() { if (topicTimerInterval) { clearInterval(topicTimerInterval); topicTimerInterval = null; } }

function getRandomQuestions(sourceQuestions, count) { const shuffled = [...sourceQuestions].sort(() => Math.random() - 0.5); return shuffled.slice(0, Math.min(count, shuffled.length)); }

function hasPaidAccess(profile) { if (!profile) return false; if (profile.role === "teacher" || profile.role === "admin") return true; if (!profile.paid_until) return false; return new Date(profile.paid_until) > new Date(); }

function isTopicsLocked() { if (!currentProfile) return true; return !hasPaidAccess(currentProfile); }

function showTopicsLockedMessage() { const lockedPanel = document.getElementById("topics-locked-panel"); const topicsGrid = document.getElementById("topics-grid"); const trainingSection = document.querySelector("#training");

if (lockedPanel) lockedPanel.style.display = "block"; if (topicsGrid) topicsGrid.style.display = "none";
...
async function startPayment(event) { if (event && typeof event.preventDefault === "function") { event.preventDefault(); } if (event && typeof event.stopPropagation === "function") { event.stopPropagation(); }

if (!currentUser) { alert("Спочатку потрібно увійти або зареєструватися."); return; }
...
function openTopic(topic) { if (!currentUser) { alert("Спочатку потрібно увійти або зареєструватися."); return; }

if (isTopicsLocked()) { alert("Доступ до тем закритий. Потрібна активна оплата на 6 місяців."); showTopicsLockedMessage(); return; }
...
function startTopic() { currentMode = "topic"; currentTopicStarted = true; startTopicTimer(); showQuestion(); }

function startExam() { if (!currentUser) { alert("Спочатку потрібно увійти або зареєструватися."); return; }

currentMode = "exam"; currentTopic = "Екзамен"; currentQuestionIndex = 0; score = 0; answered = false; currentTopicStarted = true; elapsedSeconds = 0; stopTopicTimer(); startTopicTimer();
...
function showQuestion() { answered = false;

const question = currentQuestions[currentQuestionIndex]; const trainingSection = document.querySelector("#training"); if (!trainingSection || !question) return;
...
async function checkAnswer(selectedIndex) { if (answered) return;
...
function nextQuestion() { if (currentQuestionIndex < currentQuestions.length - 1) { currentQuestionIndex++; showQuestion(); } else { if (currentMode === "exam") showExamResult(); else showResult(); } }
...
function showResult() { stopTopicTimer();

const trainingSection = document.querySelector("#training"); if (!trainingSection) return;
...
function showExamResult() { stopTopicTimer();
...
function openTraining() { if (!currentUser) { alert("Спочатку потрібно увійти або зареєструватися."); return; }

if (location.pathname.includes("training.html")) { startExam(); } else { location.href = "training.html"; }

}

async function renderStats() { const statsSection = document.querySelector("#stats"); if (!statsSection) return;

if (!currentUser) { statsSection.innerHTML =


            <h2>Статистика</h2>             <div class="panel">                 <p>Спочатку потрібно увійти або зареєструватися.</p>             </div>        
; return; }
...
async function openStats() { if (!currentUser) { alert("Спочатку потрібно увійти або зареєструватися."); return; }

if (location.pathname.includes("stats.html")) { await renderStats(); } else { location.href = "stats.html"; }

}

async function refreshProfile() { if (!currentUser) return;
...
async function registerFromForm() { const name = document.getElementById("auth-name").value.trim(); const email = document.getElementById("auth-email").value.trim(); const password = document.getElementById("auth-password").value.trim(); const message = document.getElementById("auth-message");

if (!name || !email || !password) { message.textContent = "Заповни всі поля."; message.style.color = "#fca5a5"; return; }
...
async function loginFromForm() { const email = document.getElementById("auth-email").value.trim(); const password = document.getElementById("auth-password").value.trim(); const message = document.getElementById("auth-message");

if (!email || !password) { message.textContent = "Введи email і пароль."; message.style.color = "#fca5a5"; return; }
...
async function applyAccessRulesAfterLogin() { if (!currentProfile) return;

if (location.pathname.includes("topics.html") && isTopicsLocked()) { showTopicsLockedMessage(); }

}

async function logoutUser() { const { error } = await supabaseClient.auth.signOut(); if (error) { alert("Помилка виходу: " + error.message); return; }

currentUser = null; currentProfile = null; const greeting = document.getElementById("user-greeting"); if (greeting) greeting.textContent = ""; showAuth();

}

function showAuth() { const auth = document.getElementById("auth-screen"); const app = document.getElementById("app-content");

if (auth) auth.style.display = "flex"; if (app) app.style.display = "none";

}

function showApp() { const auth = document.getElementById("auth-screen"); const app = document.getElementById("app-content");

if (auth) auth.style.display = "none"; if (app) app.style.display = "block";

}

async function loadUserProfile() { if (!currentUser) return;
...
async function checkCurrentUser() { const { data } = await supabaseClient.auth.getUser(); currentUser = data.user;

if (currentUser) { await refreshProfile(); await loadUserProfile(); showApp();
...
initReferralCode(); loadQuestions(); checkCurrentUser();
